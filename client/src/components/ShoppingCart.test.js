...
test('displays plants in cart', () => {
  // mock plants for testing purposes
  const plants = [
    {
      "name":"Peperomia Rosso",
      "id":143,"scientificName":"Peperomia caperata rosso",
      "difficulty":"easy",
      "light":"direct",
  ...
  // declare testable elements
  const shoppingCart = rtl.render(<ShoppingCart cart={plants} />);
  const snakePlant = shoppingCart.getByText(/snake plant/i);
  const stringOfDolphins = shoppingCart.getByText(/string of dolphins/i);
  const peperomiaRosso = shoppingCart.getByText(/peperomia rosso/i);
